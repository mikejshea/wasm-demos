...
fn read_level(level: i8, board: i8) -> Result<String, Box<dyn Error>> {
...
pub fn get_level(level: i8, board: i8) -> String {
    read_level(level, board).unwrap_or("".to_string())
}

#[no_mangle]
pub extern fn c_get_level(level: i8, board: i8) -> *mut c_char {
    unsafe {
        let output: Vec<u8> = read_level(level, board).unwrap().as_mut_vec().iter().map(|c| *c as u8).collect::<Vec<_>>();
        CString::from_vec_unchecked(output)
    }.into_raw()
}
